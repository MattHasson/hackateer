...
const Card: React.FC<CardProps> = ({ imageSrc, name, role, githubUrl, linkedinUrl, devpostUrl }) => {
  return (
<div className="flex flex-col items-center">
  <Image src={imageSrc} alt={name} className="lg:w-[200px] lg:h-[200px] lg:object-cover border-2 rounded-full border-berryBlush" />
  <h5 className="font-bold text-2xl lg:text-4xl text-aquamarineDream text-center">{name}</h5>
  <h6 className="font-bold text-lg lg:text-xl text-electricCitrus text-center">{role}</h6>
